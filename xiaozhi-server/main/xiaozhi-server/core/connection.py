...
class ConnectionHandler:
    ...
    async def handle_connection(self, ws: websockets.ServerConnection):
        try:
            ...
            # 客户端可在 WS 查询参数里带 voice(由设置页音色选择下发),
            # 注入为本次会话 TTS 的 private_voice,覆盖 config.yaml 里的全局 voice。
            # TTS 每连接一个实例,且 self.config 是 deepcopy 的副本,改动只影响本连接。
            # 此时 TTS 尚未实例化(在稍后的 _background_initialize 里建),无竞态。
            client_voice = self.headers.get("voice")
            if client_voice:
                tts_module = self.config["selected_module"].get("TTS")
                if tts_module and tts_module in self.config.get("TTS", {}):
                    self.config["TTS"][tts_module]["private_voice"] = client_voice
                    self.logger.bind(tag=TAG).info(
                        f"客户端指定音色: {client_voice}"
                    )
